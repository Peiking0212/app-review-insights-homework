...
- 使用工具或模型：Codex 辅助设计与实现；DeepSeek V4 Flash 真实全链路生成；Python、Pydantic、Streamlit AppTest。
- 我的目标：降低现场实时模型或网络失败风险，同时保证缓存不是伪造结果。
- 提供给 AI 的关键信息：真实美国区 Review、采集报告、各阶段已通过质量门的结构化结果、模型名称和分析目标。
- AI 生成或建议了什么：版本化快照、阶段检查点、一键恢复、真实 Demo 数据源和生成脚本。
- 我发现的问题：session_state 只能在当前进程保留；缓存若没有来源与完整性门，容易把测试数据误当真实结果。
- 我如何验证：单元测试快照往返、损坏文件、断裂阶段和不完整 Demo；真实运行 50 条评论；AppTest 切换缓存并恢复完整 Finding/Test 页面。
- 我做出的修改或取舍：只提交完整真实美国区 Demo；运行检查点不提交；原子写入；不保存 API Key；缓存模式禁用模型生成按钮。
- 测试结果：真实结果为 50 Review、7 Topic、6 Finding、5 Requirement、15 TestCase；完整自动化回归见本次提交记录。
- 关联 commit：本次真实缓存与恢复提交。
...
- 使用工具或模型：Codex 辅助实现；Apple iTunes Lookup/Public RSS 真实请求；Python 确定性采集器。
- 我的目标：让任意地区 App Store 链接都能产生可审计的美国区真实评论输入，并在外部接口受限时透明失败。
- 提供给 AI 的关键信息：Homework 原题、美国区强制要求、公开 Feed 当前响应、请求限制和现有 Review Schema。
- AI 生成或建议了什么：URL 解析、美国区规范 URL、有限分页、重试、去重、格式过滤、采集报告和 Streamlit 数据源。
- 我发现的问题：公开 Feed 当前会出现非连续空页；以“首个空页”为终止条件会漏掉后续真实评论。
- 我如何验证：模拟中国区链接、空首页、重复、畸形记录、部分失败、全空和美国区 Lookup 失败；再对题目 App 发起真实采集。
- 我做出的修改或取舍：不用模型参与数据采集；最多 10 页、每页一次有限重试和短间隔；部分页失败保留真实部分数据及限制，完全失败要求用户切换 CSV/JSON。
...
- 使用工具或模型：Codex 辅助诊断与实现；Instructor、Pydantic、Python Validator；运行时使用当前结构化输出模型。
- 我的目标：修复模型返回 31 条测试候选而固定 Schema 上限为 30 导致阶段提前失败的问题。
- 提供给 AI 的关键信息：已验证 Requirement、优先级、验收标准，以及 Python 算出的精确必需用例总数。
- AI 生成或建议了什么：每个 P0/P1 的 normal、negative、boundary 和每个 P2/P3 的 normal 场景草稿。
- 我发现的问题：“至少生成”会诱导模型为同一场景输出多个变体，固定列表上限无法表达随 Requirement 数量变化的最终约束。
- 我如何验证：构造 31 条唯一候选复现截图错误，确认 Schema 可接收、Python 收敛为 3 个必需场景、记录 28 条移除项，并保留未知 Requirement 与缺失场景阻断。
- 我做出的修改或取舍：Prompt 要求精确总数；Schema 只放宽原始候选接收上限；最终数量、ID、Review、优先级和场景覆盖仍全部由 Python 决定。
- 测试结果：阶段 5 专项测试和完整回归通过。
- 关联 commit：本次测试候选收敛修复提交。

### 2026-07-18 / Finding 证据与遗漏 Topic 综合有限修复

- 使用工具或模型：Codex 辅助实现；Instructor、Pydantic、Python Validator；运行时使用当前结构化输出模型。
- 我的目标：修复真实 100 条评论中同时出现“正面 Insight 被错误放入 supporting”和“多个 Topic 未覆盖”的组合错误。
- 提供给 AI 的关键信息：Python 定位出的错误候选、相关或遗漏 Topic、允许重新分配的 Insight 及对应原始 Review；不提供无关的正确候选证据。
- AI 生成或建议了什么：对错误候选重新判断 supporting、conflicting 或 Discovery，并补齐遗漏 Topic 的分析。
- 我发现的问题：旧逻辑只在“唯一问题为遗漏 Topic”时补分析；证据角色错误和遗漏同时出现时会安全停止，但真实演示稳定性不足。
...
- 使用工具或模型：Codex 辅助诊断与实现；Streamlit Session State、AppTest。
- 我的目标：解决侧栏快捷导航不能切换实际内容，以及阶段按钮重跑后自动返回“数据概览”的问题。
...
- 使用工具或模型：Codex 辅助实现；Altair、Streamlit。
...
- 使用工具或模型：Codex 辅助分析参考图；图像生成用于制作三套两页面视觉方向稿；Streamlit、CSS、Altair 和 AppTest 用于实现与验证。
- 我的目标：在不改动分析逻辑的前提下，降低 Streamlit 工程原型感，并让面试官更快读懂数据来源、质量门和 Review → TestCase 闭环。
- 提供给 AI 的关键信息：参考图的字体层级、留白、圆角、细描边、黑白配色，以及当前“数据概览”和 Evidence Finding 页面结构。
- AI 生成或建议了什么：黑白编辑风、靛蓝可信风和冰蓝薄荷风三套视觉方向；每套都包含数据概览与证据发现两个相同页面以便横向比较。
- 我发现的问题：全局字体规则曾覆盖 Material 图标字体，使 `keyboard_arrow_down` 显示为文字；黑色静态标签又容易被误解为已选中或可点击。
