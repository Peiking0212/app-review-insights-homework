...
- 实时失败后可以切换到 CSV/JSON 导入或“真实缓存 Demo”；内置示例不会冒充真实结果。
...
侧栏选择“真实缓存 Demo”后，系统直接加载 `data/demo_snapshot.json`，不发起 Apple 请求，也不调用模型。当前缓存于 2026-07-18 真实采集 50 条美国区公开评论，并使用 `deepseek-v4-flash` 完成全链路：

```text
50 Review → 7 Topic → 6 Finding → 5 Requirement → 15 TestCase
```

快照记录采集报告、分析目标、模型名称、清洗报告和全部结构化结果，不包含 API Key。只有 `storefront=us`、来源为 `apple_public_rss_us` 且 Topic、Finding、PRD、测试均存在的快照，才能通过 Python 校验成为 Demo。

实时运行时，每个成功阶段都会原子写入 `data/runtime/last_success.json`。该目录已加入 Git 忽略，不会把个人运行状态提交到 GitHub。失败后页面提供：
...
```powershell
.\.venv\Scripts\python.exe scripts\build_demo_snapshot.py --count 50
```

脚本只有在真实美国区评论完成全部质量门后才写入最终文件；中途失败不会留下半成品 Demo。
